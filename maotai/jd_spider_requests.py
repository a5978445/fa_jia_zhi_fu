...
class QrLogin:
    ...
    def _pinjieQrCode(self):
        base_img = Image.open("./jd.png")

        target = Image.new('RGBA', (710, 866), (0, 0, 0, 0))
        box = (166, 64, 320, 337)  # 区域
        region = Image.open(self.qrcode_img_file)
        region = region.resize((300,300))

        target.paste(base_img, (0, 0))
        target.paste(region, (198, 244))

        target.save(self.qrcode_img_file)  # 保存图片
